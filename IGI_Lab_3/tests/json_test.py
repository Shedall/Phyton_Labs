...
class JsonTestCase(unittest.TestCase):
    ...
    def test_float(self):
        ser_obj = self.js.dumps(21.34)
        des_obj = self.js.loads(ser_obj)

        self.assertEqual(des_obj, 21.34)

    def test_String(self):
        ser_obj = self.js.dumps("Test")
        des_obj = self.js.loads(ser_obj)
    ...
    def test_dict(self):
        ser_obj = self.js.dumps({12:"Test"})
        des_obj = self.js.loads(ser_obj)

        self.assertEqual(des_obj, {12:"Test"})

    def test_tuple(self):
        ser_obj = self.js.dumps((1,2,(34,56),3,"tru"))
        des_obj = self.js.loads(ser_obj)

        self.assertEqual(des_obj, (1,2,(34,56),3,"tru"))

    def test_set(self):
        ser_obj = self.js.dumps({1,2,34.556,"tru"})
        des_obj = self.js.loads(ser_obj)

        self.assertEqual(des_obj, {1,2,34.556,"tru"})

    def test_bytes(self):
        ser_obj = self.js.dumps(bytes(range(10)))
        des_obj = self.js.loads(ser_obj)
    ...
    def test_recurs_func(self):
        ser_obj = self.js.dumps(factorial)
        des_obj = self.js.loads(ser_obj)

        self.assertEqual(des_obj(5), factorial(5))

    def test_func_2(self):
        ser_obj = self.js.dumps(my_func_2)
        des_obj = self.js.loads(ser_obj)
